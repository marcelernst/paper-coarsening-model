...
class FullCoarseningModel:
    """
    This class defines the non-dimensional initial conditions of the simulation for the full coarsening model,
    based on the initial parameters, evolves the droplets and concentrations, and dumps the log into a json file.
    ...
    def __init__(self, param_dict: dict):
        ...
        self.phi_n = self.param_dict['phi_n']  # Affinity ratio between nucleoplasm and SC
        self.gamma_ds = self.param_dict['gamma_ds']  # Non-dimensional exchange rate between droplets and SC
        self.gamma_dn = self.param_dict['gamma_dn']  # Non-dim exchange rate between droplets and nucleoplasm
        self.gamma_sn = self.param_dict['gamma_sn']  # Non-dim exchange rate between SC and nucleoplasm
        self.list_l_sc = self.param_dict['list_l_sc']  # List of SC length in the cell
        self.n_sc = len(self.list_l_sc)  # Number of SCs in the cell
        self.grid_density = self.param_dict['grid_density']  # Density of the simulated grid of the SC
        self.n_sc_grid = (self.list_l_sc * self.grid_density).astype(int)  # Number of grid points per SC
        self.dx_sc_grid = self.list_l_sc / self.n_sc_grid  # Distance between grid points of SCs
        ...
        self.n_d = (self.list_l_sc * self.droplet_density).astype(int)  # Calculate number of droplets per SC
        self.m_tot_without = self.m_tot - np.sum(self.n_d) * self.m_d_init_mean  # Total HEI10 amount without droplets
        self.m_n_init = self.m_tot_without / (1 + np.sum(self.list_l_sc) * self.phi_n)  # HEI10 in nucleoplasm
        self.m_sc_init = self.phi_n * self.m_n_init  # Initial HEI10 concentration in SC

        # Define state variables
        self.m_n = np.ones(self.sample_size) * self.m_n_init  # HEI10 in nucleoplasm
        self.m_sc = np.zeros((self.sample_size, self.n_sc, np.max(self.n_sc_grid))) * np.nan  # Concentration in SC
        self.m_d = np.zeros((self.sample_size, self.n_sc, np.max(self.n_d))) * np.nan  # Droplet size
        self.x_d = np.zeros((self.sample_size, self.n_sc, np.max(self.n_d))) * np.nan  # Droplet positions
        # Index of grid point of the respective SC the droplet belongs to
        self.x_d_sc_pos = np.zeros((self.sample_size, self.n_sc, np.max(self.n_d)), dtype=int)
        for m in range(self.sample_size):
            for i in range(self.n_sc):
                self.m_sc[m, i, :self.n_sc_grid[i]] = self.m_sc_init  # homogeneous initial distribution on SC
                # Initial size distribution of droplets, based on a clipped, normal distribution
                self.m_d[m, i, :self.n_d[i]] = np.clip(np.random.normal(1, self.m_d_init_sd, self.n_d[i]), 0.1, 10)
                # Uniform droplet distribution along each SC
                self.x_d[m, i, :self.n_d[i]] = np.random.uniform(0, 1, self.n_d[i]) * self.list_l_sc[i]
                # Determine which SC grid point each droplet belongs to.
                self.x_d_sc_pos[m, i, :self.n_d[i]] = (self.x_d[m, i, :self.n_d[i]] / self.dx_sc_grid[i]).astype(int)
    ...
    def evolve(self):
        ...
        """
        now = datetime.now()
        self.t_log, self.m_n_log, self.m_sc_log, self.m_d_log, self.n_d_log = \
            evolve_numba_explicit(self.t, self.t_log, self.dt_init, self.tol,
                                  self.nu, self.nu_ds, self.nu_dn, self.phi_d, self.phi_n,
                                  self.gamma_ds, self.gamma_dn, self.gamma_sn,
                                  self.m_n, self.m_sc, self.n_sc_grid, self.dx_sc_grid, self.m_d, self.n_d,
                                  self.x_d_sc_pos)
    ...
    def dump(self, path, filename, index=0):
        ...
        results['list_l_sc'] = self.list_l_sc.tolist()
...
def load_results(path, filename, index_list=np.array([0])):
    ...
    for ii, index in enumerate(index_list):
        ...
        try:
            ...
            if results is None:
                results = load(f)
                results['list_l_sc'] = np.array(results['list_l_sc'])
...
@njit
def evolve_numba_explicit(t, t_log, dt_init, tol,
                          nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds, gamma_dn, gamma_sn,
                          m_n, m_sc, n_sc_grid, dx_sc_grid, m_d, n_d, x_d_sc_pos):
    """
    ...
    :param nu to gamma_sn: Model parameters
    :param m_n, m_sc, m_d: State variables
    ...
    n_sc = np.shape(m_sc)[1]  # get sample size

    # Define log variables
    m_n_log = np.zeros((len(t_log), sample_size)) * np.nan
    m_sc_log = np.zeros((len(t_log), sample_size, n_sc, np.max(n_sc_grid))) * np.nan
    m_d_log = np.zeros((len(t_log), sample_size, n_sc, np.max(n_d))) * np.nan
    n_d_log = np.zeros((len(t_log), sample_size, n_sc)) * np.nan

    # Write log
    t_log_idx, m_n_log, m_sc_log, m_d_log, n_d_log, coarsening_complete = \
        write_log(t_log_idx, m_n_log, m_n, m_sc_log, m_sc, m_d_log, m_d, n_d_log)
    ...
    while t < t_log[-1] and not coarsening_complete:  # evolve as long as t < maximal value of t_log and N_D>1
        ...
        m_n, m_sc, m_d, t, dt = evolve_adaptive_dt(t, dt, tol,
                                                   nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds, gamma_dn, gamma_sn,
                                                   m_n, m_sc, n_sc_grid, dx_sc_grid, n_d, x_d_sc_pos, m_d)
        # Write log
    ...
    m_n_log, m_sc_log, m_d_log, n_d_log = write_remaining_log(t_log_idx, m_n_log, m_sc_log, m_d_log, n_d_log)
    return t_log, m_n_log, m_sc_log, m_d_log, n_d_log


@njit
def write_log(t_log_idx, m_n_log, m_n, m_sc_log, m_sc, m_d_log, m_d, n_d_log):
    """
    ...
    for i in range(sample_size):
        for j in range(n_sc):
            ...
            for k in range(n_d):
                if m_d[i, j, k] > 0:
                    droplet_count += 1
            n_d_log[t_log_idx, i, j] = droplet_count

    # Check whether coarsening is complete - this triggers stop of simulation
    if np.max(np.sum(n_d_log[t_log_idx], axis=1)) == 1.0:
        coarsening_complete = True
    else:
        coarsening_complete = False
    t_log_idx += 1
    return t_log_idx, m_n_log, m_sc_log, m_d_log, n_d_log, coarsening_complete


@njit
def write_remaining_log(t_log_idx, m_n_log, m_sc_log, m_d_log, n_d_log):
    """
    ...
    return m_n_log, m_sc_log, m_d_log, n_d_log
...
def diffusion_along_sc(dt, m_sc, n_sc_grid, dx_sc_grid):
    ...
    for i in range(sample_size):
        for j in range(n_sc):
            r = dt / dx_sc_grid[j] ** 2
            for k in range(n_sc_grid[j]):
                # von Neumann Boundary condition
                if k == 0:
                    flux_sc[i, j, k] = (- m_sc[i, j, k] + m_sc[i, j, k + 1]) * r
                elif k == n_sc_grid[j] - 1:
                    flux_sc[i, j, k] = (m_sc[i, j, k - 1] - m_sc[i, j, k]) * r
                else:
                    flux_sc[i, j, k] = (m_sc[i, j, k - 1] - 2 * m_sc[i, j, k] + m_sc[i, j, k + 1]) * r
    return flux_sc


@njit
def evolve_adaptive_dt(t, dt, tol, nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds, gamma_dn, gamma_sn,
                       m_n, m_sc, n_sc_grid, dx_sc_grid, n_d, x_d_sc_pos, m_d):
    """
    Numba function to evolve the state variables, with adaptive time-stepping
    :return: updated state variables
    """
    flux_m_n, flux_m_s, flux_m_d = compute_flux_explicit(dt, nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds, gamma_dn,
                                                         gamma_sn, m_n, m_sc, n_sc_grid, dx_sc_grid, n_d,
                                                         x_d_sc_pos, m_d)
    flux_m_n_0, flux_m_s_0, flux_m_d_0 = compute_flux_explicit(dt / 2, nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds,
                                                               gamma_dn, gamma_sn, m_n, m_sc, n_sc_grid,
                                                               dx_sc_grid, n_d, x_d_sc_pos, m_d)
    flux_m_n_1, flux_m_s_1, flux_m_d_1 = compute_flux_explicit(dt, nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds, gamma_dn,
                                                               gamma_sn, m_n + flux_m_n_0, m_sc + flux_m_s_0,
                                                               n_sc_grid, dx_sc_grid, n_d, x_d_sc_pos, m_d + flux_m_d_0)

    # Adaptive time-stepping based on approach shown in https://en.wikipedia.org/wiki/Adaptive_step_size
    error = np.max(np.abs((flux_m_n_1 + flux_m_n_0) - flux_m_n))
    error_s = np.max(np.abs((flux_m_s_1 + flux_m_s_0) - flux_m_s))
    ...
    if error > 0:
        ...
        dt_factor = 2.0
    dt = 0.9 * dt * dt_factor
    return m_n, m_sc, m_d, t, dt


@njit
def compute_flux_explicit(dt, nu, nu_ds, nu_dn, phi_d, phi_n, gamma_ds, gamma_dn, gamma_sn,
                          m_n, m_sc, n_sc_grid, dx_sc_grid, n_d, x_d_sc_pos, m_d):
    """
    ...
    flux_dn = np.zeros_like(m_d) * np.nan
    flux_sn = np.zeros_like(m_sc) * np.nan
    flux_m_n = np.zeros_like(m_n) * np.nan
    ...
    for m in range(sample_size):
        for i in range(n_sc):
            for j in range(n_d[i]):
                if m_d[m, i, j] > 0:
                    phi_m_d_a = phi_d * m_d[m, i, j] ** (-nu)
                    # Exchange between nucleoplasm and droplets
                    flux_dn[m, i, j] = dt * gamma_dn * (phi_n * m_n[m] - phi_m_d_a) * m_d[m, i, j] ** nu_dn
                    # Exchange between droplets and SC
                    if nu_ds == 0.0:
                        flux_ds[m, i, j] = dt * gamma_ds * (m_sc[m, i, x_d_sc_pos[m, i, j]] - phi_m_d_a)
                    else:
                        flux_ds[m, i, j] = dt * gamma_ds * (m_sc[m, i, x_d_sc_pos[m, i, j]] - phi_m_d_a) * (
                                m_d[m, i, j] ** nu_ds)
                    # evolve droplets
                    if m_d[m, i, j] + (flux_ds[m, i, j] + flux_dn[m, i, j]) < 0.0:  # if droplet vanishes
                        # set volume to zero to avoid negative values and re-distribute exchange
                        if flux_dn[m, i, j] < 0 and flux_ds[m, i, j] < 0:
                            # distribute remaining flux to nucleoplasm and SC according to ratio
                            flux_dn[m, i, j] = - flux_dn[m, i, j] * m_d[m, i, j] / (flux_ds[m, i, j] + flux_dn[m, i, j])
                            flux_ds[m, i, j] = - (flux_dn[m, i, j] + m_d[m, i, j])
                        elif flux_dn[m, i, j] < 0:
                            # if flux from SC to droplet is positive, limit flux to nucleoplasm
                            flux_dn[m, i, j] = - (flux_ds[m, i, j] + m_d[m, i, j])
                        elif flux_ds[m, i, j] < 0:
                            # if flux from nucleoplasm to droplet is positive, limit flux to SC
                            flux_ds[m, i, j] = - (flux_dn[m, i, j] + m_d[m, i, j])
                    # compute flux from sc cells to droplets
                    flux_sc[m, i, x_d_sc_pos[m, i, j]] -= flux_ds[m, i, j] / dx_sc_grid[i]
                else:
                    flux_dn[m, i, j] = 0.0
                    flux_ds[m, i, j] = 0.0
            for j in range(n_sc_grid[i]):
                flux_sn[m, i, j] = gamma_sn * (phi_n * m_n[m] - m_sc[m, i, j]) * dt  # exchange per unit length

        flux_m_n[m] = -np.nansum(flux_dn[m])
        for i in range(n_sc):
            flux_m_n[m] -= np.nansum(flux_sn[m, i]) * dx_sc_grid[i]

    flux_sc = flux_sn + flux_sc
    flux_ds = flux_ds + flux_dn
    return flux_m_n, flux_sc, flux_ds
...
def plot_time_development(sim_results, save_to_file=False, path='', filename=''):
    ...
    mean_volumes = np.nanmean(m_d_log, axis=(1, 2, 3))
    sem_volumes = np.std(np.nanmean(m_d_log, axis=(2, 3)), axis=1) / np.sqrt(sample_size)
    ...
    mean_m_sc = np.nanmean(m_sc_log, axis=(1, 2, 3))
    sem_m_sc = np.std(np.nanmean(m_sc_log, axis=(2, 3)), axis=1) / np.sqrt(sample_size)
    ...
    mean_n_d = np.mean(np.sum(n_d_log, axis=2), axis=1)
    sem_n_d = np.std(np.sum(n_d_log, axis=2), axis=1) / np.sqrt(sample_size)
...
if __name__ == "__main__":
    ...
    filename = 'full_model_test'

    # Define parameters of full model, based on non-dimensional model in S2.A.1
    ...
    sim1 = FullCoarseningModel(param)
    # Evolve system
    sim1.evolve()
    # Dump data to file
